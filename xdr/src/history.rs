...
use xdr_history::data::{HistoryArchive, HistoryArchiveBorrowed};
use xdr_history::ic::IcBackend;
use xdr_history::History;
pub use xdr_history_common::types::*;
use std::cmp::min;
...
pub fn get_transactions(start: Nat, limit: Nat) -> Vec<TxRecord> {
    ...
    if limit > MAX_LIMIT {
        ic_cdk::api::trap(&format!("limits cannot be greater than {}", MAX_LIMIT))
    }
