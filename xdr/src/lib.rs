...
fn main() {
    ...
    match File::create("./candid/xdr.did") {
        Ok(file) => xdr_did = file,
        Err(err) => {
            println!("failed to open/create xdr.did file: {}", err);
            return
        }
    }

    if let Err(e) = xdr_did.write_all(export_candid().as_bytes()) {
