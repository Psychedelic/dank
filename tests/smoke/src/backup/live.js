...
const backupHistory = async (sdr) => {
  ...
  console.log(`Starting backup of Live SDR Transaction History`);
  console.log('');
  ...
  for (const index of transactionIds) {
    ...
    const result = await sdr.get_transaction(index);
